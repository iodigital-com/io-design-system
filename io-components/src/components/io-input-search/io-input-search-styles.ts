export function getInputSearchStyles(): string {
  return `
    :host {
      display: block;
    ...
    /* Search icon prefix — always visible */
    .search-prefix {
      display: flex;
      align-items: center;
      flex-shrink: 0;
    ...
    /* Shift label right to accommodate prefix icon */
    .input-label--has-prefix {
      left: calc(var(--io-icon-size-sm) + var(--io-space-2));
    }
    ...
    /* Suppress native browser clear button for type=search */
    ...
    /* Clear button in suffix position — only shown when value is non-empty */
    .search-clear {
      display: flex;
      align-items: center;
    ...
    :host-context([dir="rtl"]) .input-label--has-prefix {
      right: calc(var(--io-icon-size-sm) + var(--io-space-2));
      left: auto;
    }
