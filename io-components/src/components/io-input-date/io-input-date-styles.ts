export function getInputDateStyles(): string {
  return `
    :host {
      display: block;
    ...
    /*
     * Date inputs always show a browser-rendered value (dd/mm/yyyy placeholder),
     * so we keep the label permanently floated.
     */
    .input-wrapper .input-label--date-float {
      top: 0;
    ...
    /* Calendar icon suffix — reserve space */
    ...
    .input-wrapper--state-error .input-field {
      ...
      border-bottom-width: var(--io-input-date-border-error-width);
    }
