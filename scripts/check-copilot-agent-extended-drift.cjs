...
const manifestPath = path.join(agentsDir, "io-ds-ext-copilot-managed-manifest.json");

const errors = [];

function fail(code, message) {
  errors.push(`[copilot-ext-drift] ${code} ${message}`);
}
...
function main() {
  ...
  if (!Array.isArray(curated.agents) || curated.agents.length === 0) {
    fail("E_CURATED_AGENTS", "curated extended agents must be a non-empty array");
  }
  ...
  if (new Set(curatedSources).size !== curatedSources.length) {
    fail("E_CURATED_DUP", "duplicate source entries in extended curated list");
  }

  if (manifest.managedBy !== "scripts/sync-curated-agency-copilot-extended.cjs") {
    fail(
      "E_MANAGED_BY",
      `expected managedBy scripts/sync-curated-agency-copilot-extended.cjs, actual ${String(manifest.managedBy)}`,
    );
  }
  ...
  const managedExtendedFiles = entries
    .filter(
      (entry) =>
        entry.isFile() &&
  ...
  const expectedExtendedFiles = curatedSources.map((source) => managedFileName(source)).sort();

  for (const expectedFile of expectedExtendedFiles) {
    if (!managedExtendedFiles.includes(expectedFile)) {
      fail("E_MISSING_FILE", `missing managed extended file .github/agents/${expectedFile}`);
    }
  }

  for (const actualFile of managedExtendedFiles) {
    if (!expectedExtendedFiles.includes(actualFile)) {
      fail("E_EXTRA_FILE", `unexpected managed extended file .github/agents/${actualFile}`);
    }
  }
...
if (!ok) {
  ...
  console.error(`[copilot-ext-drift] FAILED with ${errors.length} mismatch(es).`);
  console.error("[copilot-ext-drift] Fix hint: run npm run agents:sync:copilot-extended");
  process.exit(1);
}

console.log("[copilot-ext-drift] OK: no extended drift detected.");
