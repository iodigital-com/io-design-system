...
//
// `ensureIconSprite()` injects a single hidden SVG sprite into the document
// body the first time it is called. Each icon name gets one `<symbol>` whose
// shape nodes are stored once — io-icon instances then render a `<svg><use>`
// reference instead of inlining the full SVG, eliminating per-instance DOM
// node duplication on pages with many icons.
//
// SSR-safe: the function guards on `typeof document`, so it is a no-op in
// Node.js (Next.js static generation). io-icon falls back to inline SVG when
// the sprite is unavailable.

const SPRITE_ID = 'io-icon-sprite';
const injectedSymbols = new Set<string>();

function buildSymbolContent(name: IoIconName): string {
  const nodes = ICON_NODES[name];
  if (!nodes) return '';
  const children = nodes
    .map(([tag, a]) => `<${tag} ${Object.entries(a).map(([k, v]) => `${k}="${v}"`).join(' ')}/>`)
    .join('');
  return `<symbol id="io-icon-${name}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${children}</symbol>`;
}

/**
 * Ensures the global SVG sprite container exists in `document.body` and that
 * the symbol for `name` has been added to it.
 *
 * SSR-safe — no-op when `document` is not defined.
 * Returns `true` when the sprite and symbol are available, `false` otherwise.
 */
export function ensureIconSprite(name: IoIconName): boolean {
  if (typeof document === 'undefined') return false;
  if (!ICON_NODES[name]) return false;

  let sprite = document.getElementById(SPRITE_ID) as SVGSVGElement | null;
  if (!sprite) {
    sprite = document.createElementNS('http://www.w3.org/2000/svg', 'svg') as SVGSVGElement;
    sprite.id = SPRITE_ID;
  ...
  if (!injectedSymbols.has(name)) {
    // Parse via innerHTML on a temporary container for cross-browser safety
    const tmp = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    tmp.innerHTML = buildSymbolContent(name);
    const symbol = tmp.firstElementChild;
    if (symbol) {
      sprite.appendChild(symbol);
    }
    injectedSymbols.add(name);
  }
...
/**
 * Returns the `<svg><use>` markup that references the sprite symbol for
 * `name`. The caller must ensure `ensureIconSprite(name)` has been called
 * first (or fall back to `getIconSvg`).
 */
export function getIconUseRef(name: IoIconName, size: number, label?: string): string {
  const ariaAttrs = label
    ? `role="img" aria-label="${escapeAttr(label)}"`
    : 'aria-hidden="true"';
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" ${ariaAttrs}><use href="#io-icon-${name}"/></svg>`;
}
