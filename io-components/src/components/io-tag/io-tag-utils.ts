...
export function getTagClassName(size: IoTagSize, color: IoTagColor, selected: boolean, disabled: boolean): string {
  return [
    'tag',
    `tag--${size}`,
    `tag--${color}`,
...
export function getTagGroupClassName(size: IoTagSize, color: IoTagColor, selected: boolean, disabled: boolean): string {
  return `tag-group tag-group--${size} tag-group--${color}${selected ? ' tag-group--selected' : ''}${disabled ? ' tag-group--disabled' : ''}`;
}
